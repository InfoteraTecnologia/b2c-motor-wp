...
(function () {
  $.simplyToast = function (e, t, n) {
    ...
    var r =
      ...
      '">';
    if (n.allowDismiss)
      r += '<span class="close" data-dismiss="alert">&times;</span>';
    r += e;
    r += "</div>";
    ...
    $(".simply-toast").each(function () {
      return (i = Math.max(
        i,
        parseInt($(this).css(n.offset.from)) + this.offsetHeight + n.spacing
      ));
    });
    var s = {
...
function saveConfig() {
  var data = {
    ...
    engine_base_url: jQuery("#engine_base_url").val(),
    base_url_api: jQuery("#base_url_api").val(),
    load_css: jQuery("#load_css").is(":checked") ? "1" : "0",
    load_tabs: jQuery("#load_tabs").is(":checked") ? "1" : "0",
    enable_hotel: jQuery("#enable_hotel").is(":checked") ? "1" : "0",
    enable_service: jQuery("#enable_service").is(":checked") ? "1" : "0",
    enable_flight: jQuery("#enable_flight").is(":checked") ? "1" : "0",
    enable_dynamic_package: jQuery("#enable_dynamic_package").is(":checked")
      ? "1"
      : "0",
    enable_flight_package: jQuery("#enable_flight_package").is(":checked")
      ? "1"
      : "0",
    enable_hotel_package: jQuery("#enable_hotel_package").is(":checked")
      ? "1"
      : "0",
    enable_bus_hotel_package: jQuery("#enable_bus_hotel_package").is(":checked")
      ? "1"
      : "0",
    enable_bus_services_package: jQuery("#enable_bus_services_package").is(
      ":checked"
    )
      ? "1"
      : "0",
  ...
  jQuery.post("./admin-ajax.php", data, function (datas) {
    var toast = $.simplyToast("Salvando configurações!", "info", {
      delay: 500,
    });
    setTimeout(function () {
      var toast = $.simplyToast("Configurações Salvas", "success", {
        delay: 2000,
      });
    }, 1000);
  });
